...
public class MusicOrganizer
{
    ...
    {
        if(validIndex(index)) {
            // stops previous track if there is one
    ...
    public void findKeyExample() {
        boolean found = false;
        int position = 0;
        String[] keys = {"A"+"B"+"C"};
        
        while(!found && position < keys.length) {
            if(keys[position].equals("B")){
                found = true;
                System.out.println("Found B at position" + position);
            }
        }
    }
